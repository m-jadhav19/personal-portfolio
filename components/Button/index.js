...
const Button = ({children, type, onClick, classes, useSelectedColor}) => {
	...
	if (type === 'primary') {
		return (
			<button
				onClick={handleClick}
				type='button'
				className={`text-sm tablet:text-base px-4 py-3 m-1 laptop:m-2 rounded-lg transition-all duration-300 ease-out min-h-11 first:ml-0 hover:scale-105 active:scale-100 link ${
	...
	return (
		<button
			...
			type='button'
			className={`text-sm tablet:text-base px-4 py-3 m-1 laptop:m-2 rounded-lg flex items-center transition-all ease-out duration-300 hover:scale-105 active:scale-100 min-h-11 tablet:first:ml-0 button-liquid-glass ${
				theme === 'dark' ? 'dark' : ''
